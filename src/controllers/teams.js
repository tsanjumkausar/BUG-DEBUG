...
module.exports.showTeam = async (req, res) => {
  ...
  const role = team.members.filter((x) => x.user && x.user._id.equals(req.user._id));
  if (role.length > 0 && role[0].role === 'Expert') {
    let bugs = team.bugs;
    res.render('teams/show', { team, bugs });
  } else if (role.length > 0 && role[0].role === 'Intermediate') {
  ...
  } else if (role.length > 0 && role[0].role === 'Nominal') {
